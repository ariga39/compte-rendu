...
The product decision is therefore to run OpenCode autonomously within the
review task inside the microVM, without approval prompts. The trusted
configuration denies permissions by default and allows the native `read`,
`grep`, `glob`, `skill`, and `submit_review` tools plus the exact static `git`
and `gh` commands needed for review. It denies edits, `webfetch`, package
managers, repository programs, builds, tests, and shell escapes. This inner
tool policy is defense in depth; the outer Sandbox boundaries below remain the
primary limits on a prompt injection or malicious repository.
...
the configured model host (`opencode.ai` by default). A deployment using a
gateway deliberately grants that gateway the same model-request access.
...
| Arbitrary outbound network                                              | Docker's host proxy makes connections using host routing. A wildcard allow can therefore expose LAN/internal services reachable from the NUC, not just the public Internet ([architecture](https://docs.docker.com/ai/sandboxes/architecture/#networking)).                                                                                                                                                                                                                  | Keep deny-by-default and allow only the configured model host/port and `api.github.com:443`. This is a high-value security boundary.                                                                                                                                                                                        |
| Model-secret misuse                                                     | A custom secret puts only a generated placeholder in the VM. However, **any sandbox process** can put that placeholder in a request to the configured host and Docker replaces it with the real value ([custom secrets](https://docs.docker.com/ai/sandboxes/configuration/credentials/#custom-secrets)). Arbitrary code therefore cannot recover the key, but can make authenticated requests to the configured model host, spend quota, and send any readable input there. | Keep the secret sandbox-scoped, one host, a job deadline/resource limits, and immediate cleanup. This capability is inherent: OpenCode itself needs it, so shell micromanagement cannot eliminate it. Custom secrets are documented as experimental.                                                                        |
| Execute repository scripts, builds, tests, hooks, or package installers | A prompt injection can request these operations, but the trusted review configuration denies them, including package managers and shell escapes. The allowed static queries can still consume bounded resources or GitHub API quota.                                                                                                                                                                                                                                         | Denied by the inner tool policy as defense in depth. Bound the remaining consequence outside OpenCode with immutable input revisions, CPU/memory/disk/wall-time limits, narrow credentials/network, output validation, and forced VM destruction.                                                                           |
| Edit the private clone                                                  | The trusted `edit` permission is denied, so OpenCode cannot modify the private clone through its edit tools. The clone remains a writable in-VM workspace, but clone mode prevents those writes from reaching the host.                                                                                                                                                                                                                                                      | Denied by the inner tool policy. Preserve the immutable base/head objects and read-only PR-context evidence outside the writable clone, and validate/publicize the result against those exact revisions. Treat a review that cannot be tied back to that evidence as an integrity failure, not as a host-security incident. |
| Read files and inspect Git history                                      | Native `read`, `grep`, and `glob` tools and the exact static Git queries inspect data already supplied to the model. They neither add a host path nor an egress destination.                                                                                                                                                                                                                                                                                                 | Allowed by the inner review policy. Full commit history and the specified read-only Git/GitHub analysis are expected review inputs; the outer microVM, mounts, credentials, and network policy remain the security boundary.                                                                                                |
...
## OpenCode's inner defense-in-depth policy

OpenCode's permissions use simple wildcard matching and the last matching rule
wins
...
The trusted review configuration sets `*` to `deny`. It allows the native
`read`, `grep`, `glob`, `skill`, and `submit_review` tools, and permits only
these static review commands through `bash`:

- `git diff`, `git grep`, `git log`, and `git show`;
- `gh pr view`; and
- `gh api graphql`.

The command rules deny output redirection, pagers, command substitution,
chaining, and other shell escapes. They also deny `edit`, `webfetch`, package
managers such as `npm` and `pnpm`, `python`, repository programs, builds, tests,
hooks, and package installation.
OpenCode is autonomous within the review task and receives no approval prompts,
but this permission policy is defense in depth rather than the primary security
boundary. The outer limits are the microVM, exact mount list, scoped
credentials, egress policy, resource/deadline envelope, retained evidence, and
terminal destruction.

This intentionally accepts residual risk inside one disposable run, even with
the inner policy:

- repository prompt injection can make permitted reads and GitHub queries spend
  the full CPU, memory, disk, wall-time, model-quota, or GitHub API budget;
- the private clone and other in-VM state remain distinct from immutable review
  evidence, so a review can be incomplete or misleading if the outer evidence
  and revision checks fail;
- permitted review queries can exercise the model credential at the configured model host
  and the built-in GitHub service's single-repository read capability at
  `api.github.com`;
- a hostile workload can crash or wedge the VM and turn the run into a failed
  cleanup/retry rather than a review result.

The inner policy reduces accidental or prompt-injected mutation but does not
replace the outer controls. None of those capabilities should cross to the NUC,
other repositories, GitHub write operations, or the home network while the
outer controls hold. Outcome quality must still be judged from the retained
result and exact base/head evidence; a successful command/session/cleanup is not
sufficient.
...
temporary one-repository token file, allows only the configured model host/port and
...
- no global/org policy or agent kit adds network allows beyond
  the configured model host/port and `api.github.com:443` (`sbx policy ls --wide` is the
  authority);
...
attempt to forbid every permitted read-only command is not.
