...
`opencode.ai:443`, bounds the run, and removes the Sandbox, secret, policy, and
temp directories afterward.
...
- no global/org policy or agent kit adds network allows beyond
  `opencode.ai:443` (`sbx policy ls --wide` is the authority);
